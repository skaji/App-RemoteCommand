...
You can append current time to each output lines with C<< --append-time >> option:
...
You can remove hostname in each output lines with C<< --no-append-hostname >> option:
